...
const getSaveFileHandle = async (filename, extension) => window.showSaveFilePicker({
...
const writeResponseToFile = async (response, handle) => {
...
export const downloadMedia = async (item) => {
  ...
  const initialFilename = `${sanitizeFilename(item.title || `${item.type}-media`)}.${urlExtension || initialExtension}`

  // Chromium's File System Access API is the most reliable way to put the
  // bytes on the user's device. The picker must be opened directly from the
  // click gesture, before any asynchronous network work begins.
  if ("showSaveFilePicker" in window && window.isSecureContext) {
    ...
    try {
      ...
      const extension = extensionFromUrl(url, item.type, contentType) || initialExtension
      const filename = `${sanitizeFilename(item.title || `${item.type}-media`)}.${extension}`
      await writeResponseToFile(response, handle)
